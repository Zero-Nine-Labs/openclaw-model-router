...
Runtime policy version 0.1.9. Classifier measurements describe routing behaviour, not answer quality or billed savings.
...
The inputs are synthetic test cases. This experiment predates the latest policy restrictions on Astra, so it is historical evidence rather than an accuracy score for version 0.1.9. Results from an earlier configuration with an incorrect effort mapping are excluded.
...
The release passed 18 portable tests and 14 compatibility tests. Compatibility tests require the validated original runtime files identified in `compat.py`, supplied outside this repository.
